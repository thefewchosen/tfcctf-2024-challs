...
long movQwordFunction(long a, long b){
    ...
    if (b < xorFunction(0xff, 0xff) || b > 255) {
        return 0;
    }
...
long cmpFunction(long index, long guessedNumber, char* filename) {
    ...
    fclose(file);
    if ((char)ch != (char)guessedNumber) {
        printf("Game over :(\n");
        exit(0);
    } else {
        startGame++;
        if (startGame == gameLength - 1) {
